...
# %% MAKE MODEL
def make_model_old():
    # In order to load model and copy weights I need to declare it here.
    model = Sequential()
    ...
    plot_model(model, to_file=output_dir + 'model_old.png', show_shapes=True)
    return model
...
# %% LOAD & COMPILE MODEL
# Transfer weights of conv layers of previous model to the new model
model_old = make_model_old()
model = make_model()
model_old.load_weights(load_model)

#Conv 1
model.layers[1].set_weights(model_old.layers[1].get_weights())
#Conv 2
model.layers[3].set_weights(model_old.layers[3].get_weights())
#Conv 3
model.layers[5].set_weights(model_old.layers[5].get_weights())
#Conv 4
model.layers[7].set_weights(model_old.layers[7].get_weights())
#Conv 5
model.layers[10].set_weights(model_old.layers[10].get_weights())
...
early_stopping = EarlyStopping(
    monitor='val_loss', min_delta=1e-4, patience=10, verbose=0, mode='auto')
